...
if (!process.env.ESCUELA_DATABASE_ID) {
  throw new Error(
    "ESCUELA_DATABASE_ID is not defined in environment variables",
  );
}
...
if (!process.env.ACTIVIDADES_DATABASE_ID) {
  throw new Error(
    "ACTIVIDADES_DATABASE_ID is not defined in environment variables",
  );
}
...
export async function getActividadesItems(): Promise<ActividadItem[]> {
  try {
    ...
    return response.results.map((page) => {
      const p = page as PageObjectResponse;
      const properties = p.properties;
      const nombreProp = properties["Título"] as {
        type: "title";
        title: Array<{ plain_text: string }>;
      };
      const destinoProp = properties["Link"] as {
        type: "url";
        url: string | null;
      };
...
async function processBlock(block: any): Promise<string> {
  ...
  if ("toggle" in block) {
    ...
    const toggleContent = await Promise.all(toggleChildren.map(processBlock));
...
export async function getNotionPage(
  pageId: string,
): Promise<{ content: string; pageTitle: string }> {
  try {
